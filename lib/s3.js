...
export async function deleteEmailFromS3(bucket, key) {
  try {
    ...
    // This prevents blocking the email processing flow if IAM permissions aren't set
    console.error(`Could not delete S3 object ${bucket}/${key}:`, error.Code || error.message)
    return false
  }
}
...
/**
 * Batch delete S3 objects (up to 1000 per call)
 * @param {string} bucket - S3 bucket name
 * @param {Array<{Key: string}>} keys - Array of objects with Key property
 * @returns {Promise<Object>} - Deletion results
 */
export async function batchDeleteS3Objects(bucket, keys) {
  ...
  try {
    const command = new DeleteObjectsCommand({
      Bucket: bucket,
      Delete: {
        Objects: keys.slice(0, 1000), // S3 limit is 1000 objects per request
    ...
    return {
      deleted: keys.length - (response.Errors?.length || 0),
      errors: response.Errors || [],
    }
  } catch (error) {
    console.error('Error batch deleting S3 objects:', error)
    throw new Error(`Failed to batch delete S3 objects: ${error.message}`)
  }
}
