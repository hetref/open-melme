 ...
 * @param {string} options.messageId - Message-ID header
 * @param {string} options.inReplyTo - In-Reply-To header for threading
...
export function buildRawMimeEmail({
  ...
  const finalMessageId = messageId || generateMessageId(domain)

  // Threading headers
  const threadingHeaders = {}
  if (inReplyTo) {
    threadingHeaders.inReplyTo = inReplyTo
  }
  if (references) {
